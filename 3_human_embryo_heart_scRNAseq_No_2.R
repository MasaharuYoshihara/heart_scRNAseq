...
dat$Sample <- factor(
  x = dat$Sample,
  levels = c("PCW6", "PCW6_ASP ET AL","PCW8", "PCW12", "PCW19")
)

Figure1A_Corrected_by_Sample <- DimPlot(
  object = dat,
  group.by = 'Sample',
  label = FALSE,
...
Figure1A_Corrected_by_Sample # W650 H450
...
# Figure_by_time <- DimPlot(
#   object = dat,
#   group.by = 'time',
#   label = FALSE,
#   label.size = 6) + ggtitle('time') + 
#   FontSize(x.title = 20, y.title = 20, x.text = 20, y.text = 20) +
#   theme(legend.text = element_text(size = 20), plot.title = element_text(size = 20))
# Figure_by_time # W700 H670
